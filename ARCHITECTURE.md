...
| 種別 | 対象 | 発動条件 | 並び順 |
| --- | --- | --- | --- |
| 構造化オンリー直行路 | 6 系統（キーワード・型・P/T 関係・部族・カード名・マナ加速） | パターン検出（開集合） | 大会 play-rate 順 |
...
「合議（RRF）は本当に必要か」という骨格への問いを、腕の寄与を消す 6 条件で実測した（2026-07-12 実施・表の基準値は当時の正準 0.811。以後の物差し更新で現行正準は 0.882 ＝ [EVAL_SCORES.md](./EVAL_SCORES.md) 参照）:
...
recall@k / precision@k / MRR / NDCG@10 と 3 段階 relevance を扱う評価ハーネス。**クエリルーター経路を通した決定的な評価**で、各機能の寄与を同一条件の A/B で測定する（GT は 34 クエリ・1,606 採点ペア。正準の看板は従来比較可能な 30 クエリ平均で維持し、2026-07-17 新設の除去直行 4 クエリは「検証終了の床」として分母外で毎回測定・未ラベル混入率ほぼ 0% を維持する運用）。
...
- 正準値の系譜（0.574 → … → 0.882）と各区間の変更内容は [EVAL_SCORES.md](./EVAL_SCORES.md)。近似検索（HNSW）は大規模な行の物理再配置で ±0.01 程度動き得ることを対照実験で確認済み——数値は「同一 DB 状態での決定的な再現」を保証する（詳細は EVAL_SCORES の注記）。
