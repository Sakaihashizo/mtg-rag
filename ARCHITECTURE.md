...
flowchart TB
    ...
    Gate -->|"はい（9 系統・下表）"| Direct["構造化オンリー直行路: SQL WHERE + 大会 play-rate 順（LLM を呼ばず数十 ms）"]
    ...
    Parsed --> Hard["SQL hard filters before search（数値・属性・型・部族・名前・ドロー枚数）"]
    ...
    CandidateTopK --> TopK[TOP-K 結果]
    CandidateTopK -.-> Reranker["Cross-encoder reranker（測って不採用・コード残置）"]
    Reranker -.-> TopK
...
直行路の判定はルーターの**前**に置かれ、該当クエリは LLM を一切呼ばず実測数十 ms で返る（LLM コストとレイテンシの両方をゼロ側に倒す）。ハードフィルタ（数値・属性条件）は検索前に SQL WHERE 句で適用し、ランキング調整（intent flags 等）は検索後に適用する。「意味検索とハード制約の分離」がこの構成の要点。役割ペナルティ（removal / counter）は手書きキーワード規則ではなく、oracle テキストから導出した構造化列（`target_types` / `removal`）による判定。reranker は 2026-07-26 の再測定（寄与 +0.024→+0.006 に縮小・採用率順の直行路には有害）を受けて**本番構成から不採用**とした——「自分が判定できない上流信号を壊さない」原則の帰結。コードと再測定手順は残置し、正準評価は reranker を通さない。

※ 本稿の「**腕**」は、候補カードを出す各検索系統（ベクトル・英語 FTS・日本語 KW・HyDE・強度・EDH）を指す内部呼称。図中 Hard ノードのハードフィルタ対象列: `cmc` / `face_cmcs` / `power` / `toughness` / `mana_producer` / `is_mana_boost` / `front_keywords`（＋NOT） / `type_line` / 部族 / カード名 / `draw_count`。

### 決定的ゲート・直行路（LLM を使わない領域）

LLM もベクトル検索も通さず SQL で解く仕組みの全体像。種別と本数:

| 種別 | 対象 | 発動条件 | 並び順 |
| --- | --- | --- | --- |
| 構造化オンリー直行路 | 9 系統（キーワード・型否定・P/T 関係・部族・カード名・収録セット・マナ加速・EDH 固有色/ブラケット・掘削×ドロー） | パターン検出（開集合） | 大会 play-rate 順（EDH 意図時は EDHREC 順） |
| 検証終了直行路 | **計 10 本**＝除去 9 ＋確定カウンター 1 | 卒業レジストリへの完全一致のみ | 検証済みレシピ順（採用率系） |
| 候補生成ゲート | ドロー枚数・機構明示除去・EDH 系（固有色・統率者名・ブラケット・banned） | 決定的検出 | 直行せず全腕の WHERE に掛かる |

#### 決定的ゲート 9 系統（構造化オンリー直行路の入口）
...
| マナ加速（2026-07-26 追加） | ルーターの意図＋「構造化に落ちない意味語が残っていない」ことの確認（土地サーチ語・コンボ語などが同居していたら発動しない） | `is_mana_boost = TRUE`（＋クエリの数値/型/フォーマット条件） |
| 収録セット（2026-07-28 追加） | 日本語セット名辞書（最長一致・曖昧な呼称は該当しうる全セットへ広く展開＝システムが勝手に一つへ絞らない） | `set_codes && ARRAY[...]`（全印刷履歴の配列・GIN。代表印刷 1 値だと再録で元セットが上書きされる偽陰性があるため配列で保持） |
| EDH 固有色・ブラケット（2026-07-08 R13） | 色語の決定的検出＋色語が無いときは統率者名辞書から固有色を解決（同名で固有色が割れる統率者は不発＝安全側） | `color_identity <@ ARRAY[...]`＋公式ブラケット文言 |
| 掘削×ドロー（2026-07-31 追加） | 「フィルタリング/掘る」系と「ドロー/引く」系の**両方の語**が原文に在るときのみ（片方だけでは沈黙） | `dig IS NOT NULL AND draw_count >= 1`（サーチ列・濾過列の共通集合） |

多義的な部族語（「人間」「悪魔」= Demon/Devil・「猿」= Ape/Monkey 等）は自動追加せず、人間のレビューを通してから辞書へ昇格させる運用（「システムは質問者の語彙を勝手に訂正しない」を原則とする）。

#### マナ加速の判定列と直行路（2026-07-26）

「マナ加速できるカード」「1マナのマナクリーチャー」のようなクエリは、正解集合が `is_mana_boost`（マナ加速かどうかの導出列）と数値・型・フォーマット条件だけで完全に定義できる。そのため意味検索を通さず SQL で返し、**並びは大会採用率順**にする。

この列は「そのカードがマナを出すか」ではなく「**マナを出すことがそのカードの本業か**」を判定する（採点規約 R15）。二段の門で導出する:

1. **文脈の門** — 起動型能力・呪文の効果・常在型・自身が戦場に出たときの誘発・マナ倍化は本業として数える。一方、**死亡したときの報酬・攻撃するたび・他の何かが起きたときの報酬**として出るマナは数えない（「宝物トークンを作る」系の札が、生け贄要員として構築で多用されるがゆえに採用率順で上位を占めてしまう問題への対処）。
2. **量の門** — 本業のマナ文について「出すマナ − 払うマナ −（土地なら 1）> 0」のみを加速とする（マナの色を変えるだけの札や、基本土地を除外するための定義）。

この形にする前は、列に二種類の誤りが混在していた——「`Add {U} or {B}`（どちらか 1 点）を 2 点と読んで合算する」ため二色土地が数百枚まぎれ、また誘発の報酬マナが本業と同格に扱われていた。**先に列を直さずに並び順だけを採用率順へ変えると、かえって悪化する**ことを対照実験で確認している（誤って加速と判定されている札ほど構築での採用率が高いため、上位に押し上げられる）。順序は「列 → 並び順」であり逆ではない、という教訓を実測で得た区間である。

#### 除去直行路（検証終了クエリの移行・2026-07-17）

除去系のうち人手採点で検証を完了した 9 クエリ（例:「クリーチャーを追放する除去」「パウパーの単体除去」「最強の単体除去」）は、上記 6 系統と同じ位置の決定的ゲートで LLM ルーターごとスキップし、役割の構造化列による SQL WHERE ＋検証済みの並び規則（機能・フォーマット系=フォーマット別大会採用率順／「最強の〜」=採用率×機能品質〔討てる対象の幅・上限の有無・テンポ効率〕の合成順）で返す。発動は**検証済みクエリの一覧（卒業レジストリ・`removal_direct.py`）への完全一致のみ**。似たクエリへの汎化は検証を持たないため行わず、一致しなければ従来のハイブリッド経路に落ちる（誤発動=有害・取り逃し=無害の非対称設計。専用の安全試験 38 ケースで誤発動ゼロを担保）。検証成績はファミリー 9 クエリ平均 NDCG@10 0.987（[EVAL_SCORES.md](./EVAL_SCORES.md) 参照）。「クリーチャーを破壊する除去」だけはハイブリッド経路が上回ったため移行していない。
...
枚数指定のドロークエリ（「カードを2枚引く」「draw two cards」等）では、oracle テキストの**命令形「Draw N card(s)」だけ**を数えた導出列 `draw_count`（可変枚数＝Draw X / that many / for each は `draw_x`）に対し、`draw_count >= N OR draw_x` の WHERE を候補生成の全系統（ベクトル / FTS / 強度腕 / HyDE 腕）へ掛ける（導出は `enrich_draw.py`）。パーサーは誘発の条件文（Whenever you draw ...）と置換文（If you would draw ...）に現れる draw を数えない。これは「破壊不能」カードが destroy の字面で除去クエリの上位に浮いていた問題の、ドロー版への対処である（「引いたとき」の報酬カードや「引く代わりに」の置換カードが、ドロー源の顔で上位に浮く）。「手札補充」「フィルタリング」系クエリには掛けない（引かずに手札へ加える選別カードも正解に含む＝「補充」は「引く」より広い語、という人手採点の実測に基づく線引き）。効果は同一 DB 状態のゲート ON/OFF 対照走行で +0.011（draw two cards 0.394→0.798）・非ドロークエリの変動ゼロ（誤発動ゼロ）を実測。安全試験 87 ケースを併設（`test_draw_gate.py`）。

#### EDH（統率者戦）系の決定的ゲート（固有色・統率者名・ブラケット・banned）

統率者戦の文脈を持つクエリには、`attr_sql` 経由で全腕＋直行路に同時に掛かる決定的ゲート群を用意している:

- **固有色ゲート**: 「ゴルガリカラーの除去」等は `color_identity ⊆ クエリ色集合` のハードゲート（部分集合判定＝単色や無色のカードも「そのデッキに入るか」で通す）。commander banned はブラケット指定の有無に関わらず除外。
- **統率者名→固有色の解決（2026-07-20）**: 「アトラクサで使える除去」のように**統率者名で色を指定する**クエリを、伝説のクリーチャー 3,125 枚から構築した名前辞書（正式名・読点短縮名・英語名）で固有色に解決して同じ ⊆ ゲートへ流す。検索ツールに統率者名を打つ人の圧倒的多数は構築文脈、という事前確率で既定 ON とし、**敵対文脈（「〜対策」「相手の〜」等の閉集合語彙）はその名前の近傍だけで判定して発動を抑える**——クエリ全域で判定すると「アトラクサを使ったデッキで相手のウラモグに対応できるカード」のような複合クエリで、後半の「対応」が前半の正当な発動まで殺すため。同名で色が割れる場合（オムナス等）は不発＝安全側。専用の非対称安全試験 22 ケース（最悪形＝無色の統率者名で ⊆∅ が発動し無色カードしか出なくなる事故、を必須ケースに）。
- **ブラケットゲート**: 「ブラケット2で使える〜」は公式 Game Changers リスト（`game_changer` 列・2026-02-09 改定の 53 枚と一致）をハードゲートで除外。ブラケット 3 以上・無指定では発動しない（公式ルールどおり）。

EDH 向けの評価は 2026-07-24 に専用トラックとして稼働開始した（統率者名軸の 10 クエリ・人手採点 100 ペア・**分母は本線の看板と混ぜず別掲**）。詳細は [EVAL_SCORES.md](./EVAL_SCORES.md)。

### LLM の出力を検証・補正する層（プロバイダ非依存）
...
**信頼境界は一点に置く（2026-07-30〜31 の再設計）**。上記のガードは当初 LLM 応答の解析部（経路ごと）に置かれていたが、「ガードを通らない経路（評価キャッシュ）で同じ幻出が再発する」事故を実測した——ガードを経路ごとに貼る設計は、貼り忘れた経路で必ず再演する。対処として検証の関所を **searcher が SQL を組む直前の一点**へ移し、どの経路（ローカル 7B・Gemini キャッシュ・Nova）から来ても同じ検証を通る形にした。同じ思想で、LLM が自己申告していた除去・カウンターの意図フラグ（mode）は**廃止**し、原文からの決定的な辞書検出に一本化した（LLM の申告で立てた意図が誤発動し、検索結果を汚す事故の実測から）。LLM の出力のうち決定的に検出できるものは読まない方向へ段階的に縮小しており、**代替が利かないのは「言い換えの正規化」と「HyDE 文の生成」の二つ**——これは腕の切除実験（埋め込み全廃で本線 0.94→0.71）で実測した、埋め込み経路の担当領域と一致する。
...
- **デッキとカードの多対多関係は `deck_cards` で正規化**。スクレイプ由来の名前ゆれ（`[]` 接頭辞・分割カードの旧区切り ` / `・両面カードの表面名）を正規化して `card_id` を解決し、紐付け率 51.8% → **99.96%**（修理時点。以後デッキの日次増加に伴い現在は 99.95%＝1,000,229 / 1,000,773。残の未解決は Planechase 次元カード等、カード DB の対象外＝正当な NULL）。
- **生データは JSONB/バルク、検索のホットパスで使う属性だけ列に昇格**（`legalities` / `card_faces_json` は JSONB で保持）。横長スカスカなテーブルを避ける方針。
- **リーガル / 非リーガルを分離**。Vintage 非リーガル（un 系・Alchemy・リバランス版）2,779 枚を `*_nonlegal` テーブルへ退避し、検索対象コア（31,635 枚）をクリーンに保つ。
...
| `edhrec_rank` | EDHREC 由来の人気度ランク | **EDH 意図クエリの候補腕と直行路の並び順で使用中**（EDH 向け評価トラックは 2026-07-24 稼働開始・GT は本線と別掲＝[EVAL_SCORES.md](./EVAL_SCORES.md) 参照） |
...
reembed の間も検索を止めないため（TRUNCATE からの全件再構築で長時間かかる）、Docker 上で PostgreSQL Primary/Standby を構築し、フラグファイルで Standby に切り替える機構を実装。**Read Replica を Zero-downtime Data Refresh に応用したパターンの検証**であり、商用 HA ではない。
...
「合議（RRF）は本当に必要か」という骨格への問いを、腕の寄与を消す 6 条件で実測した（2026-07-12 実施・表の基準値は当時の正準 0.811。以後の物差し更新で現行正準は 0.937 ＝ [EVAL_SCORES.md](./EVAL_SCORES.md) 参照）:
...
recall@k / precision@k / MRR / NDCG@10 と 3 段階 relevance を扱う評価ハーネス。**クエリルーター経路を通した決定的な評価**で、各機能の寄与を同一条件の A/B で測定する（GT は 34 クエリ・1,702 採点ペア。EDH 100 ペア・意味検索 60 ペアの別掲トラックを併設。正準の看板は従来比較可能な 30 クエリ平均で維持し、2026-07-17 新設の除去直行 4 クエリは「検証終了の床」として分母外で毎回測定・未ラベル混入率ほぼ 0% を維持する運用）。
...
- 正準値の系譜（0.574 → … → 0.937）と各区間の変更内容は [EVAL_SCORES.md](./EVAL_SCORES.md)。近似検索（HNSW）は大規模な行の物理再配置で ±0.01 程度動き得ることを対照実験で確認済み——数値は「同一 DB 状態での決定的な再現」を保証する（詳細は EVAL_SCORES の注記）。
