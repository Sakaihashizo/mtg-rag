...
[README](./README.md) が「現在地のスナップショット」であるのに対し、本書は「**どう作られているか・なぜそう判断したか**」の詳細を記録する。評価スコアの系譜は [EVAL_SCORES.md](./EVAL_SCORES.md)、データモデルの全列・索引は [DATA_MODEL.md](./DATA_MODEL.md)、時系列の開発記録は [DEVLOG.md](./DEVLOG.md) を参照。
...
## AWS 構成の運用詳細（構成決定の要約と維持費表は README 参照）
...
精度差は小さく速度差は約 1.5 倍のため SMALL を採用。LARGE（1024d）は初期検証で SMALL を上回らなかったため見送り（標準指標での再測定を予定）。
