...
「合議（RRF）は本当に必要か」という骨格への問いを、腕の寄与を消す 6 条件で実測した（2026-07-12 実施・表の基準値は当時の正準 0.811。以後の物差し更新で現行正準は 0.803 ＝ [EVAL_SCORES.md](./EVAL_SCORES.md) 参照）:
...
recall@k / precision@k / MRR / NDCG@10 と 3 段階 relevance を扱う評価ハーネス。**クエリルーター経路を通した決定的な評価**で、各機能の寄与を同一条件の A/B で測定する（GT は n=30 クエリ・1,117 採点ペア・未ラベル混入率 0.0%）。
...
- 正準値の系譜（0.574 → … → 0.803）と各区間の変更内容は [EVAL_SCORES.md](./EVAL_SCORES.md)。近似検索（HNSW）は大規模な行の物理再配置で ±0.01 程度動き得ることを対照実験で確認済み——数値は「同一 DB 状態での決定的な再現」を保証する（詳細は EVAL_SCORES の注記）。
