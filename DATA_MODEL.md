...
本書のスキーマ・件数は実 DB（PostgreSQL 18 + pgvector）から確認したもの（2026-06-19 時点）。

カードデータは総数 33,711 件。うち vintage 非リーガル（un 系・Alchemy・リバランス版）2,779 件を `*_nonlegal` テーブルへ退避し、検索対象コアは **30,932 件**。以下の件数はこのコア側。
...
| テーブル | 行数 | 役割 |
|---|---:|---|
| `mtg_cards_v2` | 30,932 | カード本体（コア） |
| `mtg_embeddings_small_v2` | 30,932 | multilingual-e5-small の埋め込み（384 次元） |
| `mtg_embeddings_base_v2` | 30,932 | multilingual-e5-base の埋め込み（768 次元） |
...
カード本体と 1 対 1（件数一致 30,932 / 30,932）。構造を分離することで、構造化列の更新時に再ベクトル化を避ける。
