...
| `mtg_embeddings_small_v2` | 31,635 | multilingual-e5-small の埋め込み（384 次元） |
| `mtg_embeddings_base_v2` | 31,635 | multilingual-e5-base の埋め込み（768 次元） |
| `deck_list` | 12,006 | デッキ（本線 4F 6,990 ＋ precon 2,731 ＋ Duel Commander 214 ＋ Vintage 1,087 ＋ Pauper 984・source 列で系統分離） |
| `deck_cards` | 408,542 | デッキ収録カード明細（card_id 解決 99.9%+・未解決 131 行は次元カード等の正当な NULL） |
| `card_format_strength` | 4,281 | カード×フォーマット別 play-rate 事前集計（導出テーブル） |
| `card_cooccurrence` | 2,941,426 | カード共起（precon/本線 4F/EDH/Vintage/Pauper を source 列で分離。embed_text の "Often used with" は本線 4F 分のみが出所） |
| `eval_runs` | 46 | 評価実行ログ（内部用） |
...
現在 4,281 行（2,930 カード × 4 フォーマット: Legacy / Modern / Pioneer / Standard）。「最強」系クエリの GT 機械採点に加え、**検索側でも使用中**（2026-07-05 接続済み）: (1) tournament_boost の加算元（旧 `tournament_score` 列からの配線替え）、(2) play-rate 上位を候補生成に加える「強度腕」、(3) 構造化オンリー直行路の並び順（play_decks 降順）。大会データ由来のランキング信号の品質はこの表の集計品質に直結する。
