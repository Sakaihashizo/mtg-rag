...
- 33,711 件のカードデータ（うち検索対象 30,932 件・非リーガルは別テーブルに退避）を対象にした日英 RAG 検索（pgvector HNSW + 英語 FTS + 日本語 KW + HyDE の 4 系統ハイブリッド + 標準 RRF, k=60）
...
| 構造化メタデータフィルタ（cmc / face_cmcs / power / toughness / マナ生成） | 実装済み・効果は評価に未反映 |
...
| 評価フレームワーク（recall@k / precision@k / MRR / NDCG@10） | 実装済み・暫定ベースライン取得済み（n=24・searcher 直呼び）。ルーター経路でも試行したが、データ修正で embedding が変わったためクリーンな再ベースラインを取得中 |
| Cross-encoder reranker | 予定（評価のルーター経路化・再ベースライン後に A/B） |
...
flowchart TB
    ...
    Router -->|JSON 出力| Parsed["search_query + intent flags + type_filter(validated) + 数値/属性フィルタ + HyDE text"]
    Parsed --> Hard["SQL hard filters before search: cmc / power / toughness / mana_producer"]
...
- **リーガル / 非リーガルを分離**。vintage 非リーガル（un 系・Alchemy・リバランス版）2,779 枚を `*_nonlegal` テーブルへ退避し、検索対象コア（30,932 枚）をクリーンに保つ。
...
これは**ハード制約の充足率であって relevance（NDCG 等）の改善ではない**。relevance への効果は、評価をクエリルーター経路に通した上で別途測定する必要がある（未実施）。
...
注意: ground truth を SMALL_V2 の top10 からプールしているため **recall@10 ≈ 0.95 は構造上ほぼ天井で識別力が低く、主指標にしない**（改善を語るなら NDCG@10 / recall@5 / MRR）。また、この測定は検索器を直接呼んでおり**クエリルーター経路（構造化フィルタ含む）を通っていない**。reranker や構造化フィルタの改善 A/B は、評価をルーター経路に通して再ベースラインを取得した後に行う予定。評価セットも小さい。
...
特に、**マナ総量・カードタイプ・色・フォーマットなどの構造化条件を含むクエリでは、ベクトル検索のみでは条件が曖昧化する**ことがある。この課題への対応として、自然言語クエリから構造化可能な条件（`mana_value`・パワー・タフネス・`type_filter`・マナ生成の有無）を LLM クエリプランナーで抽出し、SQL のハードフィルタで厳密に絞り込み、曖昧な意味部分のみをベクトル検索・FTS・HyDE・RRF に委ねる**構造化メタデータフィルタを実装済み**。ただし**その効果はまだ評価ベースラインに反映されていない**（現在の評価ハーネスは検索器を直接呼び、クエリルーター経路を通らないため）。
...
| --- | --- |
| カードデータ総数 | 33,711 |
| ├ 検索対象（リーガル・embedding 済み） | 30,932 |
| └ 非リーガル（un 系 / Alchemy 等・別テーブルに退避） | 2,779 |
| SMALL / BASE embedding | 各 30,932 件（vector 384 / 768 次元） |
...
- **評価をクエリルーター経路に通し、ルーター経路での再ベースラインを取得する**（現ベースラインは検索器直呼びのため、構造化フィルタ等の効果が測定に反映されていない。経路が変わるため新旧の数値は直接比較しない）
- 再ベースライン取得後、**Cross-encoder reranker を導入し同一ハーネスで A/B 定量化**
- 評価セットの拡充（現状 n=24）と標準指標での継続測定
- 取り込み済みメタデータの活用検証（produced_mana の色指定対応、edhrec_rank / game_changer の検索品質への寄与測定）
