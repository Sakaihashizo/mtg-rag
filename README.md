...
| 構造化メタデータフィルタ（mana_value / power / toughness / マナ生成） | 実装済み・効果は評価に未反映 |
...
| 評価フレームワーク（recall@k / precision@k / MRR / NDCG@10） | 実装済み・ベースライン取得済み（n=24・暫定。クエリルーター経路は未通過） |
...
flowchart TB
    ...
    Parsed --> Hard["SQL ハードフィルタ (検索前・WHERE 句)<br/>cmc / power / toughness / mana_producer"]
    ...
    FTSJA --> RRF
    VectorHyDE --> RRF
    RRF --> Soft["ランキング調整 (検索後)<br/>type_filter / tournament_boost / counter_mode / removal_mode"]
    Soft --> TopK[TOP-K 結果]
...
flowchart TB
    ...
    subgraph refresh["メタデータ更新パイプライン（構想・未実装）"]
        eb["EventBridge (日次・新セット検知)"] --> sfn["Step Functions (更新 → Wait 21日 → 更新)"]
        sfn --> lamA["Lambda A (VPC外) / Scryfall バルク取得 → S3"]
        lamA --> s3b[("S3 ブリッジ")]
        s3b --> lamB["Lambda B (VPC内) / S3 Gateway エンドポイント → DB 更新"]
    end
    lamB -.-> proxy
```
...
- **代替案の棄却**: S3 ロード型デモ構成を、前提条件（DB 常時課金の回避）が消えた時点で採用見送りとした（前述）。
