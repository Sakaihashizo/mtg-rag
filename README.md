...
- **crisp な条件は SQL の門で解き、意味検索は曖昧な意味にだけ使う**: 構造化列（キーワード能力・フォーマット・型・マナ総量）だけで答えの集合を完全に定義できるクエリは、ベクトル検索・HyDE・reranker もルーター LLM 呼び出しも通らず、SQL 直行路（大会 play-rate 順・決定的・数十 ms）で返す。キーワード系 6 クエリはこの経路で NDCG@10 1.000。誤って直行路へ入る事故（誤発動）はゼロを必須とする非対称のゲート安全試験（34 ケース・決定的・全緑）付き
...
| 構造化オンリー直行路（crisp クエリの SQL 完結） | 実装済み。キーワード系 6 クエリ NDCG@10 1.000。ルーター LLM 呼び出しもスキップ（ゲート安全試験 34 ケース・誤発動ゼロ） |
