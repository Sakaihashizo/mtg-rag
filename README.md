...
### 1. 評価方法論 — 検索品質 NDCG@10 **0.932** を、緩い採点で盛らずに出す

34 クエリ・1,588 の人手採点ペアによる**決定的な評価基盤**を先に作り、検索のあらゆる変更を同一条件の A/B で判定してきた（初期ベースラインは 0.574）。採点基準は明文化し、構成変更のたびに現れる未採点カードは採点を広げて、混入率ほぼ 0% を維持している。

**評価基準そのものも監査対象**にしている。2026-07 には、経路 A/B が「未採点カードを 0 点扱いする系統誤差」で歪むことを実測して採点済みのみで測る近似指標を併記し、機能クエリの採点基準も「利用者が求める順位」と整合する形に再定義した（規約 R2'）。**「緩い採点なら高く出るが、その数字に意味はない」**が運用原則で、弱点クエリのスコアも隠さず記録している。2026-07 末には統率者戦（EDH）向けの評価トラックを新設し、**分母を既存の看板と混ぜず別掲**とした（初回 10 クエリ・人手採点 100 ペア・初値 NDCG@10 0.975）。「採点の網がまだ浅い初期値は高く出る」という逆向きのバイアスも、注記した上で運用している。系譜と内訳は [EVAL_SCORES.md](./EVAL_SCORES.md)。
...
| 評価 GT（3 段階 relevance・人手採点） | 本線 1,588 ペア / 34 クエリ＋EDH 別掲 100 ペア / 10 クエリ |
