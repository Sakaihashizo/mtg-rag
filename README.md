...
### 1. 評価方法論 — 検索品質 NDCG@10 **0.939** を、緩い採点で盛らずに出す

34 クエリ・1,606 の人手採点ペアによる**決定的な評価基盤**を先に作り、検索のあらゆる変更を同一条件の A/B で判定してきた（初期ベースラインは 0.574）。採点基準は明文化し、構成変更のたびに現れる未採点カードは採点を広げて、混入率ほぼ 0% を維持している。

**評価基準そのものも監査対象**にしている。2026-07 には、経路 A/B が「未採点カードを 0 点扱いする系統誤差」で歪むことを実測して採点済みのみで測る近似指標を併記し、機能クエリの採点基準も「利用者が求める順位」と整合する形に再定義した（規約 R2'）。**「緩い採点なら高く出るが、その数字に意味はない」**が運用原則で、弱点クエリのスコアも隠さず記録している。2026-07 末には統率者戦（EDH）向けの評価トラックを新設し、**分母を既存の看板と混ぜず別掲**とした（初回 10 クエリ・人手採点 100 ペア・初値 NDCG@10 0.975 → 採点基準の是正後 0.938）。「採点の網がまだ浅い初期値は高く出る」という逆向きのバイアスも、注記した上で運用している。系譜と内訳は [EVAL_SCORES.md](./EVAL_SCORES.md)。
...
| 除去直行路（検証終了クエリ 9 本・SQL 直行＝LLM 不使用） | 実装済み（安全試験・誤発動ゼロ） |
| 評価フレームワーク（決定的・人手採点 GT） | 実装済み・運用中 |
| 大会 play-rate のランキング接続（強度腕＋役割ゲート） | 実装済み |
| EDH（統率者戦）対応 | 固有色・ブラケット・統率者名ゲート実装済み。専用評価トラック稼働開始（別掲 GT・初回 10 クエリ） |
...
| 大会データの定期リフレッシュ | 夜間バッチ稼働中（毎日 03:00 に差分スクレイプ→採用率再集計。何度走っても結果が変わらない冪等設計・稼働初日 35 パス全緑） |
...
- **既知の弱点クエリ（隠さず指標に残す）**: コンボ（0.59）とドローフィルタ（0.59・クエリ文言の再考候補）、1マナのマナクリーチャー（0.71）。ドロー系は「引く」の構造化列（draw_count / draw_x）＋枚数ゲート＋採点規約の明文化（R14・補足a/b）＋日英対称の是正で解決した（draw two cards 0.39→0.95・カードを2枚引く／インスタントでドロー 1.000）。緩い採点なら高く出るが、その数字に意味はないと判断している。
...
| 大会デッキ（MTGTop8・構築 6 フォーマット＋Duel Commander） | 13,307 件（夜間バッチで毎日成長） |
| 多人数 Commander デッキ（Moxfield・ブラケット別取得） | 1,232 件 |
| デッキ内カード行の card_id 解決率 | 694,547 行中 99.96% |
| カード×フォーマット別 play-rate 集計 | 構築 6,257 行＋EDH 15,377 行（物理分離） |
| 評価 GT（3 段階 relevance・人手採点） | 本線 1,606 ペア / 34 クエリ＋EDH 別掲 100 ペア / 10 クエリ |
...
- **AWS デプロイ**（次段階・統率者関連クエリの検証を仕上げてから）: 残りは Lambda コンテナ化と実デプロイ。検証項目は [ARCHITECTURE.md](./ARCHITECTURE.md) に明記
- **EDH 評価トラックの被覆拡大と「質の層」**: 定義に合うことの先にある「統率者とのかみ合わせ」への適合（例: 打ち消しの副産物でマナが出るカードを「キナンで使えるマナ加速」の上位に出さない）。役割内の実勢データ・脅威との整合判定が候補
- コンボ等、構造化列に写せない意味クエリの改善
