...
> 現時点では再現性のあるワンコマンドセットアップは未整備。以下は開発中環境の実行手順メモであり、今後 `requirements.txt` / `docker-compose` / `scripts/` への整理を予定している。この通りに実行しても環境差で動かない可能性がある。

```bash
# 1. 環境変数
cp .env.example .env   # DB 接続情報を設定（認証情報はコミットしない）

# 2. データ取り込み（Scryfall 等の公式 API から各自取得）
