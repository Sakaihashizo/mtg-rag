...
- その後**採点規約を明文化して GT を 1,017 採点ペアへ拡張・再採点**し（NDCG@10 0.682）、「最強」系クエリは大会 play-rate による機械採点へ切り替え。**厳格 GT の正準値は NDCG@10 0.673 / precision@5 0.900**（未ラベル混入 0.3%。旧 GT の 0.673 との数値一致は偶然で、採点基準が別物のため直接比較しない）。クエリ別内訳は [EVAL_SCORES.md](./EVAL_SCORES.md)
...
- MTGTop8 の**大会デッキ 6,990 件**を取り込み、名寄せ（card_id 解決 99.96%）の上でカード×フォーマット別の採用デッキ数（play-rate）を事前集計。「最強」「環境」系クエリの接地に向けた土台（**検索への接続は未実装**）
...
| 評価フレームワーク（recall@k / precision@k / MRR / NDCG@10） | 実装済み。**ルーター経路・3 段階 GT・決定的**でベースライン取得＋各機能の A/B 済み（n=30・GT 1,017 採点ペア・未ラベル混入率 0.3%）。採点規約（機能・機構ベース）を明文化して運用。「最強」系は大会 play-rate による機械採点（循環評価の回避策込み） |
| Cross-encoder reranker | **実装済み**（bge-reranker-v2-m3・top-10 並べ替え）。A/B で precision@5 / MRR / NDCG@10 改善 |
| 大会デッキデータ（MTGTop8） | 6,990 デッキ取り込み・名寄せ済み（card_id 解決 99.96%）・フォーマット別 play-rate 事前集計済み。**検索への接続は未実装** |
...
recall@k / precision@k / MRR / NDCG@10 と 3 段階の relevance（`human_grade` 0/1/2）を扱う評価ハーネスを実装。**クエリルーター経路を通した決定的な評価**で、各機能の寄与を同一条件の A/B で測定している（GT は n=30 クエリ・1,017 採点ペア・未ラベル混入率 0.3%）。
...
さらに「最強」「純粋に強い」等の品質ランキング語は、カードテキストを読んでも答えが決まらない（メタゲームでの実績の話）ため、**大会デッキ 6,990 件から集計したフォーマット別 play-rate による機械採点**へ切り替えた。ただし役割つきクエリ（「最強の単体除去」等）は「機能判定 AND play-rate 閾値」とし、データ単独ではど真ん中（grade 2）に昇格させない——play-rate は「カードの強さ」であって「役割としての強さ」ではない（除去としては並のカードが、別の役割の強さで play-rate 上位に来る事例を実データで確認済み）。この厳格化を経た**現行の正準値は NDCG@10 0.673 / precision@5 0.900 / MRR 0.958（未ラベル混入 0.3%）**。

- **0.682 → 0.673 は劣化ではなく「採点を正直にしたコスト」**。特に「純粋に強いカード」が 0.642 → 0.360 へ下がり、現行のランキング信号が古い事前集計列に依存し役割を見ていない、という実弱点が GT に隠されず露出した。この信号の配線替えが次の最優先レバー（「今後の展望」参照）。
- **循環評価の回避**: GT を play-rate だけで作ると、検索側のランキング信号（同じ大会データ由来）と正解の情報源が同一になり、NDCG が同語反復化する。役割つきクエリに機能判定のゲートを残すのはこのため。機械採点した行には集計スナップショット日付を記録し、メタゲームの変動に対する再現性を確保している。
- 旧 GT の 0.673 と数値が一致するのは偶然（採点基準が別物のため直接比較しない）。
- この再採点の過程で、**人手採点が実装バグを 2 件発見**した（クエリ拡張の部分文字列衝突・分割カードの日本語テキスト混入。詳細は「対応した技術課題」）。数字を取るための評価が、実際にはデバッグ装置として機能した。
- クエリ別の内訳は [EVAL_SCORES.md](./EVAL_SCORES.md) を参照（キーワード系クエリは構造化フィルタで解けて 1.0 近くに飽和する一方、除去・「最強」系の「順位の質」を競うクエリが 0.3〜0.7 に分布しており、改善の主戦場が後者であることが分かる）。
...
| 評価 GT（3 段階 relevance・人手採点＋「最強」系 57 ペアは play-rate 機械採点） | 1,017 採点ペア / 30 クエリ |
...
- **ランキング信号の配線替え（最優先）**: 現行の tournament_boost は古い事前集計列に依存しており、「純粋に強いカード」NDCG@10 0.360 の主因。フォーマット別 play-rate 集計（card_format_strength）への接続に置き換える。あわせて **boost 系クエリは reranker を通さない**——rerank on/off の A/B で、reranker が自分では判定できない上流のランキング信号を並べ替えで壊すことを確認済み（他の 25 クエリでは reranker が NDCG@10 を +0.02 稼ぐため、全体では維持）。retrieval が変わるため、再プール＋採点ラウンドをセットで行う（coverage bias 対策）。**play-rate（人気）とクエリ意図の品質（例: 単体除去としての質）が乖離する事例を実データで確認済み**のため、盲目的なブーストにしない方針は維持する。
