...
resource "aws_lambda_function" "api" {
  ...
  # 並列実行の予約は**しない**（2026-07-31 実測で不可能と判明）: このアカウントの
  # Lambda 同時実行の総枠は 10 で、未予約分の最低保証も 10＝何を予約しても保証を
  # 割るため PutFunctionConcurrency が 400 で拒否される（新アカウントの既定枠）。
  # 実害なし——アカウント総枠 10 がそのまま並列の蓋であり、API GW スロットル
  # （2 req/s × 実行約 1.3 秒 ＝ 定常同時実行 約 2.6）が実効の上限を作っている。
  # 総枠の引き上げを申請する日が来たら、このコメントを予約設定に置き換えること。
