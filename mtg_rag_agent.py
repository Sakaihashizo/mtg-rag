...
出力: {{"search_query": "カードを2枚引く", "hyde_text": "Draw two cards. This spell costs {{1}} less if you control a creature.", "tournament_boost": true, "removal_mode": false, "counter_mode": false, "type_filter": null}}

入力: 「環境で強いクリーチャーを3枚」
出力: {{"search_query": "環境で強いクリーチャー", "hyde_text": "2/2 creature with haste. When this creature enters, draw a card. Ward {{2}}.", "tournament_boost": true, "removal_mode": false, "counter_mode": false, "type_filter": "Creature"}}

入力: 「純粋に強いカウンター呪文を3枚選んでそれぞれ理由を教えて」
出力: {{"search_query": "純粋に強いカウンター呪文", "hyde_text": "Counter target spell. Draw a card.", "tournament_boost": true, "removal_mode": false, "counter_mode": true, "type_filter": "Instant"}}

入力: 「モダンの最強単体除去を教えて」
出力: {{"search_query": "最強の単体除去", "hyde_text": "Destroy target creature. This spell costs {{1}} less if the creature entered this turn.", "tournament_boost": true, "removal_mode": true, "counter_mode": false, "type_filter": null}}

入力: 「1マナのマナクリーチャー」
出力: {{"search_query": "マナを生むクリーチャー", "hyde_text": "Creature. {{T}}: Add one mana of any color.", "tournament_boost": false, "removal_mode": false, "counter_mode": false, "type_filter": "Creature", "mana_producer": true, "cmc_min": 1, "cmc_max": 1}}

入力: 「2マナ以下のカウンター呪文」
出力: {{"search_query": "カウンター呪文", "hyde_text": "Counter target spell.", "tournament_boost": false, "removal_mode": false, "counter_mode": true, "type_filter": "Instant", "cmc_max": 2}}
...
def rewrite_query(query: str, api_key: str) -> tuple[str, str, bool, bool, bool, str | None]:
    """
    Gemini を使ってクエリを解析し検索用クエリ・HyDEテキスト・フラグを返す。
    戻り値: (search_query, hyde_text, tournament_boost, removal_mode, counter_mode, type_filter)
    失敗時は元のクエリと空文字・False フラグを返す。
    """
    ...
    try:
        ...
        return (search_query, hyde_text, tournament_boost,
                removal_mode, counter_mode, type_filter, filters)
    except Exception:
        return query, "", False, False, False, None, {}
...
def process_question(searcher, question, fmt, top_k, api_key,
    ...
    if use_rewrite:
        ...
        search_query, hyde_text, tournament_boost, removal_mode, counter_mode, type_filter, filters = rewrite_query(
            question, api_key
        )
        flags = []
