...
CASES = [
    ...
    # ─ 収録セット: 入口には**置かない**（2026-07-31 A/B 実測・agent 側コメント参照。
    #   旧経路は直行の並びに HyDE が乗っており、入口でルーターを切ると並びが変わる
    #   ＝HyDE 剥がしの本人裁定待ちに抵触。裁定が出たら True 側へ動かすこと）─
    ("灯争大戦のプレインズウォーカー", False),
    ("灯争大戦のカード", False),
    ("エルドレインのクリーチャー", False),
    ("灯争大戦の強いカード", False),          # 強さ判定＝意味の残余あり→ルーターへ
    ("灯争大戦の3マナのカード", False),       # 数値抽出はルーターの仕事
    # ─ dig∧draw（2026-07-31 入口追加・門関数と同一判定）─
    ("ドローしながらフィルタリングできるカード", True),
    ("ドローしながら除去できるカード", False),  # 門の誤発動ゼロ試験と同じ側
    ("フィルタリングできる青いカード", False),  # draw 語なし＝門は沈黙
]
